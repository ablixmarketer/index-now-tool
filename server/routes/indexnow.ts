...
export const handleBulkPing: RequestHandler = async (req, res) => {
  try {
    const validation = BulkPingRequestSchema.safeParse(req.body);
    ...
    const summary = {
      ...
      succeeded: results.filter(r => r.status === 200 || r.status === 202).length,
  ...
  try {
    ...
    const results = await Promise.all(
      selectedEngines.map(engineId => 
        pingSingleUrl(url, engineId, 'update')
      )
    );
...
async function pingSingleUrl(
  ...
  try {
    ...
    const response = await fetch(pingUrl, {
      ...
      headers: engineId === 'bing' ? {} : {
        'Content-Type': 'application/json',
        'User-Agent': 'IndexNow-Ping-Console/1.0'
      },
      body: engineId === 'bing' ? undefined : JSON.stringify({
        host: new URL(url).hostname,
        key: INDEXNOW_KEY,
        urlList: [url],
        ...(INDEXNOW_KEY_LOCATION && { keyLocation: INDEXNOW_KEY_LOCATION })
      })
    });
